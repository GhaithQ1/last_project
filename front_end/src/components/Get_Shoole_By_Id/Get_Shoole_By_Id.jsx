...
const [loading ,SetLoading] = useState(false)
  useEffect(() => {
    ...
    .then((res) => {
      ...
      SetLoading(false) // لا تعمل شي إذا ما وصل الاي دي
      })
      .catch((error) => {
  ...
  const sendFriendRequest = (id) => {
    ...
    axios
      .post(`${API}/auth/Send_friend_request/${id}`, {}, headers).then((res) => {
                ...
                <div className="profile_all">
                  ...
                  <div className="buttom_F">
                    ...
                    {AllMydata?.Friend_requests?.some(
                      ...
                      <div className="accept_G">
                        <div className="button_req">
                          {Accept[GetDataById?._id] === "accepted" ? (
                            <p>
                              You are friends now
                          ...
                          ) : Accept[GetDataById?._id] === "rejected" ? (
                            <p>
                    ...
                    {!AllMydata?.friends?.some(
                      ...
                      <button
                        className={`websit_button ${sentRequests[GetDataById?._id] || GetDataById?.followers?.some((f) => f.friend === AllMydata?._id) ? "follow_active" : ""}`}
                        disabled={sentRequests[GetDataById?._id]} // حتى ما ينضغط مرتين
                        onClick={() => sendFriendRequest(GetDataById._id)}
                      >
                        {GetDataById.role === "employee"
                          ? sentRequests[GetDataById?._id] || GetDataById?.followers?.some((f) => f.friend === GetDataById?._id) ? "Followed" : "Follow"
                          : sentRequests[GetDataById?._id] || GetDataById?.followers?.some((f) => f.friend === GetDataById?._id) ? "You have sent a friend request" : "Add a friend"}
                      </button>
                    ) : null}
                  </div>
                </div>
